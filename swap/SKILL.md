...
## Test workflow (devnet)

Run `npm run test:devnet` from the `swap/` directory to execute the v2 integration harness (`test/v2-flows.test.mjs`). The harness generates a throw-away keypair at `swap/test/.devnet-keypair.json` (gitignored, chmod 0600), verifies the cluster genesis matches devnet, airdrops 2 SOL via up to 3 retries, then executes four phases in sequence: **A** buy_v2 bonding-curve buy (0.01 SOL), **B** sell_v2 (half the tokens gained), **C** buy_exact_quote_in_v2 (5_000_000 lamports), and **D** claim_cashback_v2. Each phase prints a Solscan link on confirmation. Pass `--reset` to delete cached mint files and start fresh. Phase C may emit a documented warning on devnet if the mainnet-only fee-recipient accounts are absent; this is expected behaviour and does not count as a failure. The public devnet RPC airdrop endpoint is IP-rate-limited; if airdrop fails after 3 retries (30-second backoff each), pre-fund the test wallet printed in the `[USER]` log line using `https://faucet.solana.com` or a Helius/QuickNode devnet faucet, then re-run.
