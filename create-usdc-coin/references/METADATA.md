...
1. **Your product:** Implement or reuse an upload flow (image → IPFS / object storage, then JSON → IPFS) and pass the final JSON URL to `scripts/build-create-usdc-coin-tx.mjs` as `--metadata-uri`.
...
Pass the `metadataUri` to the create script:
